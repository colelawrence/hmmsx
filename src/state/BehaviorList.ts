...
import { ObserverListProps } from "../observer";
...
export class ObservableList<E> implements IObservableList<E> {
  // for dotJSX
  jsx: (props: ObserverListProps<E>) => Dot.DotNode = (
    props: ObserverListProps<E>
  ) =>
    createDotNode(this._order.asObservable(), {
      ...props,
      // @ts-ignore
...
const isNotEmpty = <A>(e: A | ItemPlaceholders) =>
  e !== INIT_ITEM && e !== REMOVED_ITEM;
  ...
  const nextItem = (e: any) =>
    e !== INIT_ITEM && e !== REMOVED_ITEM
      ? props.nextItem
        ...
        : e
      : null;
  return createDotNode("div", {
    children: behs.map(obs =>
      createDotNode(obs.pipe(map(nextItem)), {
        key: obs[observerKey],
...
export class BehaviorList<E> extends ObservableList<E> {
  ...
  private _nextAppendItems(items: E[]) {
    ...
    for (let i = 0; i < itemsLength; i++) {
      this._order.value[i + len] = new Behavior<E | ItemPlaceholders>(items[i]);
    }
  ...
  constructor(init?: E[]) {
    super(
      new Behavior<Behavior<E | ItemPlaceholders>[]>([
        new Behavior<E | ItemPlaceholders>(INIT_ITEM)
      ])
    );
    this.capacity = 1;
    ...
    return this._order.pipe(
      flatMap(behs => {
        if (behs.length === 0) return empty;
          ...
          return combineLatest(...behs).pipe(
            map(arr => {
